...
def initial_setup():
    ...
    open_port(80)
    open_port(443)
    if bool(db.get('fqdns', {})) or bool(config().get('fqdn', {})):
        set_state('letsencrypt.configured')
    else:
        status_set('blocked', 'Waiting for domain name, either through unitdata or through config')
...
def register_server():
    configs = config()
    fqdns = db.get('fqdns', configs.get('fqdn', []))
